...
document.addEventListener('DOMContentLoaded', () => {
  // 1. 卡片深度背景与原帖精译悬浮放大弹窗 (Floating Card Modal)
  initCardDetailModal();
  ...
  document.querySelectorAll('.copy-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      const brief = card.querySelector('.item-brief, .insight-text')?.textContent?.trim() || '';
      const summary = card.querySelector('.item-summary, .drawer-summary')?.textContent?.trim() || '';
      const author = card.querySelector('.item-author strong, .author-label strong')?.textContent?.trim() || '';
      const source = card.querySelector('.source-link, .source-action')?.href || '';
      ...
      try {
        ...
        btn.innerHTML = `<span style="font-size:11px;font-weight:700;color:var(--color--brand-color);line-height:1">✓</span>`;
        btn.setAttribute('title', '已复制精编到剪贴板');
        setTimeout(() => {
...
// 7.5. 卡片深度背景与原帖精译悬浮放大弹窗 (Floating Card Modal)
function initCardDetailModal() {
  const modalBackdrop = document.getElementById('card-detail-modal-backdrop');
  const modalBody = document.getElementById('card-detail-modal-body');
  const closeBtn = document.getElementById('close-card-detail-modal');

  if (!modalBackdrop || !modalBody) {
    // 降级兼容：如果页面未包含弹窗结构，则执行就地折叠展开
...
function initPosterShare() {
  ...
  document.querySelectorAll('.share-poster-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      const title = card.querySelector('.item-title, .card-headline')?.textContent?.trim() || '';
      const category = card.querySelector('.item-category, .category-chip')?.textContent?.trim() || '前沿快讯';
      const author = card.querySelector('.item-author strong, .author-label strong')?.textContent?.trim() || '';
      const authorRole = card.querySelector('.item-author span, .author-label span')?.textContent?.trim() || '';
      const brief = card.querySelector('.item-brief, .insight-text')?.textContent?.trim() || '';
      const summary = card.querySelector('.item-summary, .drawer-summary')?.textContent?.trim() || '';
